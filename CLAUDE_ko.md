...
**릴리즈 버전 관리 — 자동 버전 증가:**
`release/<version>` 브랜치 생성 전, 항상 다음 버전을 자동으로 결정합니다:
1. `git tag --sort=-v:refname | head -1` 실행으로 최신 semver 태그 확인 (예: `v0.1.0`).
2. 태그가 없으면 `package.json`의 `version` 필드를 기준으로 사용.
3. 기본적으로 **patch** 번호를 증가합니다 (예: `v0.1.0` → `v0.1.1`). 신규 기능 포함 시 **minor**, 브레이킹 체인지 시 **major** 증가.
4. 결정된 버전을 브랜치명(`release/<version>`)과 git 태그(`v<version>`) 모두에 사용합니다.
5. 기존 태그를 절대 재사용하거나 덮어쓰지 않습니다. 계산된 버전이 이미 태그되어 있으면 다시 증가합니다.
6. `README.md`를 새 버전과 주요 변경사항이 반영되도록 갱신한 뒤, 릴리즈 브랜치를 머지하기 전에 별도 커밋으로 추가한다.
